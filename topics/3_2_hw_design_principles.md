...
Where each style of hardware tends to win, plotted for applications rather than for network architectures. The axes are the two properties that decide the outcome, how sparse the activity still is when it reaches the datapath and how much work each event sets off. An event-driven design needs both at once, so the lower right corner, where events are rare but each one does little work, still favors a clocked accelerator, and the band between the two regions is where hybrid event and clocked designs sit. The dashed arrow marks that these coordinates belong to the deployed application, so sparsifying a model moves it across the map with no change to the hardware at all. Positions are illustrative rather than measured.
```
...
:width: 100%
:align: center
:alt: An energy calculator for four fully connected integrate-and-fire layers. Each layer has a neuron-count input and a spike-density control. The result separates expected SENeCA energy per 10 millisecond time step into active synaptic integration, a density-independent neuron-update floor, emitted-spike capture, and a fixed 30 microwatt single-core static term, then reports total energy and average power against an all-active reference.

Interactive estimate of SENeCA neuron-processing energy and power for four fully connected integrate-and-fire layers. The synaptic term uses 12.7 pJ per BF16 one-event synaptic operation. The neuron-update pass costs 12.1 pJ per neuron per time step, with another 1.1 pJ only when the neuron emits a spike. The paper's approximately 30 µW of static/leakage power for one core adds 300 nJ to every fixed 10 ms time step. The static version shows a 256-128-64-10 network at 10% spike density: dynamic processing requires 58.4 nJ, and the single-core static term brings the total to 358 nJ per step, or 35.8 µW. Each additional mapped core would add another static term. Input encoding, the projection into Layer 1, RISC-V and NoC overhead, and external-memory traffic are excluded [@tang2023open].
```

The event-processing savings can therefore be large on naturally sparse signals such as audio and event-based vision, though per-event overhead and static power always set a floor. An interactive energy calculator, built on measured SENeCA per-instruction and per-component costs, lets a reader watch the density-dependent synaptic term in {eq}`eq:event-energy` rise above the density-independent neuron-update and static-power floors as more neurons fire [@tang2023open].
