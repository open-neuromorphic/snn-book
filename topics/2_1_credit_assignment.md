...
```{note}
This chapter helps to understand the purpose of the coming chapters.
...
Here's the example in the figure: say we get some input ($1$) and that we want that input to result in an output of $2$.
In this example, $w$ is set to ${2}$, so the output $1 * w_1 = 1 * 2 = 2$ is off by one.
We can solve this by adjusting the linear weight from $2 \to 1$, which gives $1 * 1 = 1$: we reached our goal!
...
The membrane potential carried from step to step, which makes SNNs especially hard:
...
And distance is subtle: a step of $0.1$ in one weight may barely change the network's behaviour, while the same step in another may ruin it.
...
It rescales the update by the [Fisher information metric](https://en.wikipedia.org/wiki/Fisher_information_metric), so a unit step means "change the output distribution by this much" no matter how the parameters happen to be parameterised. 
The update becomes invariant to reparameterisations.
...
The ambiguity we have discussed so far is a question of *correctness*: which component, at which moment, deserves the blame.
...
These objections motivate the alternatives that follow, some keep backprop's form but swap the transposed weights for fixed random ones (*feedback alignment* [@lillicrap2016random], *direct feedback alignment* [@nokland2016direct]).
...
- **Eligibility-based**: each synapse keeps a running *eligibility trace* of its recent activity, so a later global signal (an error or a reward) can find the synapses responsible: the *three-factor* rule of neuroscience [@gerstner2018eligibility]. This is more than heuristic. *e-prop* [@bellec2020solution] shows the exact BPTT gradient of {eq}`eq:bptt-gradient` factorises into just such a local trace times a top-down signal (see [](#biologically_inspired_training)).
...
Now that you have multiple options, the practical question is *when to reach for which*.
...
- **Do you have a differentiable loss and enough memory to backpropagate?** Reach for a **gradient-based** method (surrogate gradients, [](#surrogate_gradients)). It is the default when it applies, because it uses the error most efficiently.
...
They are separated here only where the distinction changes *when* you would reach for them.
...
| Plasticity-based (local/Hebbian) | You need an on-chip, biologically plausible, online rule | A precise global error must be minimised | Low; local and online |
...
| Direct optimization | The parameter space is tiny or no gradient exists | The parameter space is large | Scales poorly with dimension |
