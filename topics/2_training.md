...
This topic covers the complete training workflow for SNNs: from understanding how credit assignment flows (through layers) to practical and efficient training setups, tailored to your use case.
...
We use the simplest possible spiking neuron: a single **integrate-and-fire (IF)** unit (from [](#foundations)) driven by a constant input that it scales by one weight $w$. Our goal is modest — make the neuron fire at a *target rate*. With a poor initial $w$, the rate is wrong; training nudges $w$ until it matches the target.
